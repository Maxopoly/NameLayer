	...
	public void handle(String ticket, ConnectedServer sendingServer, JSONObject data, UUID executor, Group group) {
		if (group == null) {
			sendReject(ticket, MergeGroups.REPLY_ID, sendingServer,
					MergeGroups.FailureReason.ORIGINAL_GROUP_DOES_NOT_EXIST);
			return;
		}
		Group targetGroup = getGroupTracker().getGroup(data.getString("groupToDelete"));
		if (targetGroup == null) {
			sendReject(ticket, MergeGroups.REPLY_ID, sendingServer,
					MergeGroups.FailureReason.TARGET_GROUP_DOES_NOT_EXIST);
			return;
		}
		...
		synchronized (lowerID) {
			synchronized (upperID) {
				if (group.equals(targetGroup)) {
					sendReject(ticket, MergeGroups.REPLY_ID, sendingServer,
							MergeGroups.FailureReason.CANNOT_MERGE_INTO_SELF);
					return;
				}
				...
				if (!getGroupTracker().hasAccess(targetGroup, executor, permNeeded)) {
					sendReject(ticket, MergeGroups.REPLY_ID, sendingServer,
							MergeGroups.FailureReason.NO_PERMISSION_TARGET_GROUP, repValues);
					return;
				}
				...
				getGroupTracker().addLogEntry(group,
						new MergeGroup(System.currentTimeMillis(), executor, targetGroup.getName()));
