...
public class UnlinkGroupsHandler extends GroupRequestHandler {
	...
	public void handle(String ticket, ConnectedServer sendingServer, JSONObject data, UUID executor, Group group) {
		if (group == null) {
			sendReject(ticket, UnlinkGroups.REPLY_ID, sendingServer, UnlinkGroups.FailureReason.ORIGINAL_GROUP_DOES_NOT_EXIST);
			return;
		}
		String targetGroupName = data.getString("targetGroup");
		Group targetGroup = getGroupTracker().getGroup(targetGroupName);
		if (targetGroup == null) {
			sendReject(ticket, UnlinkGroups.REPLY_ID, sendingServer, UnlinkGroups.FailureReason.TARGET_GROUP_DOES_NOT_EXIST);
			return;
		}
		synchronized (group) {
			if (group.equals(targetGroup)) {
				sendReject(ticket, UnlinkGroups.REPLY_ID, sendingServer, UnlinkGroups.FailureReason.CANNOT_UNLINK_SELF);
				return;
			}
			...
			if (!getGroupTracker().hasAccess(group, executor, permNeeded)) {
				...
				sendReject(ticket, UnlinkGroups.REPLY_ID, sendingServer, UnlinkGroups.FailureReason.NO_PERMISSION_ORIGINAL_GROUP, repValues);
				return;
			}
			if (!getGroupTracker().hasAccess(targetGroup, executor, permNeeded)) {
				Map<String, Object> repValues = new HashMap<>();
				repValues.put("missing_perm", NameLayerPermissions.LINK_GROUP);
				sendReject(ticket, UnlinkGroups.REPLY_ID, sendingServer, UnlinkGroups.FailureReason.NO_PERMISSION_TARGET_GROUP, repValues);
				return;
			}
			GroupRank originalRank = group.getGroupRankHandler().getRank(data.getInt("originatingRank"));
			if (originalRank == null) {
				sendReject(ticket, UnlinkGroups.REPLY_ID, sendingServer, UnlinkGroups.FailureReason.ORIGINAL_RANK_DOES_NOT_EXIST);
				return;
			}
			GroupRank targetRank = group.getGroupRankHandler().getRank(data.getInt("targetRank"));
			if (targetRank == null) {
				sendReject(ticket, UnlinkGroups.REPLY_ID, sendingServer, UnlinkGroups.FailureReason.TARGET_RANK_DOES_NOT_EXIST);
				return;
			}
