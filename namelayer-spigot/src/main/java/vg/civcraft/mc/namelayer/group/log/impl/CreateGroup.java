...
public class CreateGroup extends LoggedGroupAction {
	...
	public ItemStack getGUIRepresentation() {
		...
		ItemAPI.setDisplayName(is,
				String.format("%s%s%s created %s%s", ChatColor.GOLD, getPlayerName(), ChatColor.GREEN, ChatColor.WHITE, name));
		enrichItem(is);
	...
	public String getChatRepresentation() {
		return String.format("%s%s%s created %s%s", ChatColor.GOLD, getPlayerName(), ChatColor.GREEN,
