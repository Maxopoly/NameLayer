...
public class SetPassword extends PlayerCommandMiddle{
	...
	public boolean execute(CommandSender sender, String[] args) {
		...
		NameLayerMetaData meta = NameLayerPlugin.getInstance().getNameLayerMeta().getMetaData(g);
		meta.setPassword(password);
		
		p.sendMessage(ChatColor.GREEN + "Password has been successfully set to: " + password);
		return true;
	}
