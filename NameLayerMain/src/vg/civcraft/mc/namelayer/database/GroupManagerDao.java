...
public class GroupManagerDao {
	...
	public void checkUpdate() {
		...
		if (ver == 0) {
			...
			log(Level.INFO,
					"Performing database update to version 1!\n"
			...
			db.execute("delete fm.* from faction_member fm where not exists " // deletes
																				// any
																				// non
																				// faction_id
																				// entries.
			...
			db.execute("alter table db_version change db_version db_version int not null;"); // for
																								// some
																								// reason
																								// db_version
																								// may
			...
			db.execute("insert into permissions (group_id, role, tier) "
					...
					+ "from faction_id f;");
			db.execute("insert into permissions (group_id, role, tier) "
					+ "select f.group_id, 'MEMBERS', " + "'DOORS CHESTS' "
					+ "from faction_id f;");
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version one took "
		...
		if (ver == 1) {
			...
			db.execute("create table if not exists faction_id("
					...
					+ "group_name varchar(255)," + "primary key(group_id));");
			/*
			 * In the faction table we use group names. This is important
			 * because when merging other groups it will create multiple same
			 * group_names within the faction_id table. The benefits are that
			 * when other tables come looking for a group they always find the
			 * right one due to their only being one group with a name.
			 */
			db.execute("create table if not exists faction("
					+ "group_name varchar(255)," + "founder varchar(36),"
			...
			db.execute("create table if not exists faction_member("
					+ "group_id int not null," + "member_name varchar(36),"
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(
					NameLayerPlugin.getInstance(), new Runnable() {
						...
						@Override
						public void run() {
							Group g = getGroup(NameLayerPlugin
									.getSpecialAdminGroup());
							if (g == null)
								createGroup(
										NameLayerPlugin.getSpecialAdminGroup(),
										null, null);
							else {
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version two took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 2) {
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version three took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 3) {
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version four took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 4) {
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version five took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 5) {
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version six took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 6) {
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version seven took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 7) {
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version eight took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 8) {
			long first_time = System.currentTimeMillis();
			log(Level.INFO,
					"Database updating to version nine, fixing datatypes.");
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version nine took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 9) {
			long first_time = System.currentTimeMillis();
			log(Level.INFO,
					"Database updating to version ten, adding table to keep track of name changes");
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version ten took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		if (ver == 10) {
			long first_time = System.currentTimeMillis();
			log(Level.INFO,
					"Database updating to version eleven, reworking permission system");
			...
			PreparedStatement permInit = db
					.prepareStatement("insert into permissionByGroup(group_id,role,perm_id) values(?,?,?);");
			try {
				Map<String, Integer> permIds = new HashMap<String, Integer>();
				PreparedStatement getOldPerms = db
						.prepareStatement("select * from permissions");
				...
				while (res.next()) {
					...
					for (String p : perms) {
						if (!p.equals("")) {
							if (p.equals("BLOCKS")) {
								// this permission was renamed and now includes
								// less functionality than previously
								p = "REINFORCE";
							}
			...
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version eleven took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}
		...
		if (ver == 11) {
			long first_time = System.currentTimeMillis();
			log(Level.INFO,
					"Database updating to version twelve, reworking player type system");
			db.execute("create table if not exists groupPlayerTypes(group_id int not null,type_id int not null,type_name varchar(40) not null, parent_id int, blacklist boolean not null, primary key(group_id,type_id));");
			...
			db.execute("insert into groupPlayerTypes (group_id,type_id,type_name,blacklist) select group_id, 0, 'OWNER',false from faction_id;");
			db.execute("insert into groupPlayerTypes (group_id,type_id,type_name,parent_id,blacklist) select group_id, 1, 'ADMINS',0,false from faction_id;");
			db.execute("insert into groupPlayerTypes (group_id,type_id,type_name,parent_id,blacklist) select group_id, 2, 'MODS',1,false from faction_id;");
			db.execute("insert into groupPlayerTypes (group_id,type_id,type_name,parent_id,blacklist) select group_id, 3, 'MEMBERS',2,false from faction_id;");
			db.execute("insert into groupPlayerTypes (group_id,type_id,type_name,parent_id,blacklist) select group_id, 4, 'DEFAULT',0,false from faction_id;");
			db.execute("insert into groupPlayerTypes (group_id,type_id,type_name,parent_id,blacklist) select group_id, 5, 'BLACKLISTED',4,true from faction_id;");
			
			//retire old blacklist
			db.execute("insert into faction_member (group_id,member_name,type_id) select group_id, member_name ,5 from blacklist;");
			db.execute("drop table blacklist");
			// old permission table is no longer used, so delete it
			db.execute("drop table permissions");
			ver = updateVersion(ver, plugin.getName());
			log(Level.INFO,
					"Database update to Version twelve took "
							+ (System.currentTimeMillis() - first_time) / 1000
							+ " seconds.");
		}

		log(Level.INFO, "Database update took "
	...
	public void initializeProcedures() {
		...
		db.execute("create definer=current_user procedure deletegroupfromtable("
				...
				+ "delete from faction where group_name = groupName;" + "end;");
		...
		db.execute("create definer=current_user procedure mergeintogroup("
				...
				+ "update ignore faction_member fm " // move all members from
														// group From to To
				...
				+
				/*
				 * "UPDATE faction_member fm " // update roles in dest on
				 * overlaps to be from merged group +
				 ...
				 */
				"DELETE fm.* from faction_member fm " // Remove those "overlap"
														// members left behind
														// by IGNORE
				...
				+
				/*
				 * "DELETE FROM subgroup s " // If this was a subgroup for
				 * someone, unlink. subgroups to new group. +
				 * "WHERE sub_group_id IN " // TODO: might be double effort? +
				 * "(SELECT group_id from faction_id where group_name = tomerge);"
				 * +
				 */// handled using unlink
				"UPDATE subgroup s " // If it was a subgroup's supergroup,
										// redirect
				+ "SET s.group_id = destID "
				+ "WHERE s.group_id IN "
				+ "(SELECT group_id from faction_id where group_name = tomerge);"
				+ "delete from faction where group_name = tomerge;"
				+ // Remove "faction" record of From
				"update faction_id fi " // Point "faction_id" records to TO's
										// Name instead
				+ "inner join faction_id fii on fii.group_name = tomerge "
				+ "set fi.group_name = groupName "
				+ "where fi.group_id = fii.group_id;" + "end;");
	...
	private PreparedStatement createGroup, getGroup, getGroupById,
			getAllGroupsNames, deleteGroup;

	private PreparedStatement addMember, getMembers, removeMember,
			updatePassword, updateOwner;

	private PreparedStatement addSubGroup, getSubGroups, getSuperGroup,
			removeSubGroup;
	...
	private PreparedStatement addAutoAcceptGroup, getAutoAcceptGroup,
			removeAutoAcceptGroup;

	private PreparedStatement setDefaultGroup, changeDefaultGroup,
			getDefaultGroup, getAllDefaultGroups;

	private PreparedStatement loadGroupsInvitations, addGroupInvitation,
			removeGroupInvitation, loadGroupInvitation,
			loadGroupInvitationsForGroup;

	private PreparedStatement getGroupNameFromRole, updateLastTimestamp,
			getPlayerType, getTimestamp;

	private PreparedStatement getGroupIDs;

	private PreparedStatement logNameChange, checkForNameChange;

	private PreparedStatement addPermission, getPermission, removePermission,
			registerPermission, getPermissionMapping;

	private PreparedStatement getPlayerTypeRoot, addPlayerType,
			removePlayerType, renamePlayerType, getChildrenPlayerTypes;

	private PreparedStatement removePlayerTypeMember,
			removeInvitationByPlayerType;

	public void initializeStatements() {
		version = db
				.prepareStatement("select max(db_version) as db_version from db_version where plugin_name=?");
		updateVersion = db
				.prepareStatement("insert into db_version (db_version, update_time, plugin_name) values (?,?,?)");

		createGroup = db.prepareStatement("call createGroup(?,?,?,?)");
		getGroup = db
				.prepareStatement("select f.group_name, f.founder, f.password, f.discipline_flags, fi.group_id "
						+ "from faction f "
						+ "inner join faction_id fi on fi.group_name = f.group_name "
						+ "where f.group_name = ?");
		getGroupIDs = db
				.prepareStatement("SELECT f.group_id, count(DISTINCT fm.member_name) AS sz FROM faction_id f "
		...
		//have to make sure here we only get it for groups in which the player isnt blacklisted
		getAllGroupsNames = db
				.prepareStatement("select f.group_name from faction_id f "
						+ "inner join faction_member fm on f.group_id = fm.group_id "
						+ "inner join groupPlayerTypes gpt on fm.type_id = gpt.type_id "
						+ "where fm.member_name = ? and gpt.blacklist = 0");
		...
		getMembers = db
				.prepareStatement("select fm.member_name,fm.type_id from faction_member fm "
						+ "inner join faction_id id on id.group_id = fm.group_id where id.group_name=?");
		removeMember = db
				.prepareStatement("delete fm.* from faction_member fm "
						+ "inner join faction_id fi on fi.group_id = fm.group_id "
						+ "where fm.member_name = ? and fi.group_name =?");

		// So this will link all instances (name/id pairs) of the subgroup to
		// all instances (name/id pairs) of the supergroup.
		addSubGroup = db
				.prepareStatement("INSERT INTO subgroup (group_id, sub_group_id) "
		...
		// This undoes the above. It unlinks all instances (name/id pairs) of
		// the subgroup from all instances (name/id pairs) of the supergroup.
		removeSubGroup = db
				.prepareStatement("DELETE FROM subgroup "
		...
		// This lists all unique subgroups (names) for all instances (name/id
		// pairs) of the supergroup.
		getSubGroups = db
				.prepareStatement("SELECT DISTINCT sub.group_name FROM faction_id sub "
		...
		// This lists all unique supergroups (names) which are parent(s) for all
		// instances (name/id pairs) of the subgroup.
		// I expect most implementations to ignore if this has multiple results;
		// a "safe" implementation will check.
		getSuperGroup = db
				.prepareStatement("SELECT DISTINCT f.group_name FROM faction_id f "
		...
		// returns count of unique names, but not (name / id pairs) of all
		// groups.
		countGroups = db
				.prepareStatement("select count(DISTINCT group_name) as count from faction");

		// returns count of unique names of groups owned by founder
		countGroupsFromUUID = db
				.prepareStatement("select count(DISTINCT group_name) as count from faction where founder = ?");

		mergeGroup = db.prepareStatement("call mergeintogroup(?,?)");

		updatePassword = db
				.prepareStatement("update faction set `password` = ? "
						+ "where group_name = ?");

		updateOwner = db.prepareStatement("update faction set founder = ? "
				+ "where group_name = ?");

		addAutoAcceptGroup = db
				.prepareStatement("insert into toggleAutoAccept(uuid)"
						+ "values(?)");
		getAutoAcceptGroup = db
				.prepareStatement("select uuid from toggleAutoAccept "
						+ "where uuid = ?");
		removeAutoAcceptGroup = db
				.prepareStatement("delete from toggleAutoAccept where uuid = ?");

		setDefaultGroup = db
				.prepareStatement("insert into default_group values(?, ?)");

		changeDefaultGroup = db
				.prepareStatement("update default_group set defaultgroup = ? where uuid = ?");

		getDefaultGroup = db
				.prepareStatement("select defaultgroup from default_group "
						+ "where uuid = ?");
		getAllDefaultGroups = db
				.prepareStatement("select uuid,defaultgroup from default_group");

		loadGroupsInvitations = db
				.prepareStatement("select uuid, groupName, type_id from group_invitation");

		addGroupInvitation = db
				.prepareStatement("insert into group_invitation(uuid, groupName, type_id) values(?, ?, ?) on duplicate key update type_id=values(type_id), date=now();");

		removeGroupInvitation = db
				.prepareStatement("delete from group_invitation where uuid = ? and groupName = ?");

		loadGroupInvitation = db
				.prepareStatement("select type_id from group_invitation where uuid = ? and groupName = ?");

		loadGroupInvitationsForGroup = db
				.prepareStatement("select uuid,type_id from group_invitation where groupName=?");

		// Gets the "most recent" updated group from all groups that share the
		// name.
		getTimestamp = db
				.prepareStatement("SELECT MAX(faction.last_timestamp) FROM faction "
						+ "WHERE group_name = ?;");

		// updates "most recent" of all groups with a given name.
		updateLastTimestamp = db
				.prepareStatement("UPDATE faction SET faction.last_timestamp = NOW() "
						+ "WHERE group_name = ?;");

		getPlayerType = db
				.prepareStatement("SELECT type_id FROM faction_member "
						+ "WHERE group_id = ? " + "AND member_name = ?;");
		logNameChange = db
				.prepareStatement("insert into nameLayerNameChanges (uuid,oldName,newName) values(?,?,?);");
		checkForNameChange = db
				.prepareStatement("select * from nameLayerNameChanges where uuid=?;");

		addPermission = db
				.prepareStatement("insert into permissionByGroup(group_id,type_id,perm_id) select g.group_id, ?, ? from faction_id g where g.group_name = ?;");
		getPermission = db
				.prepareStatement("select pg.perm_id from permissionByGroup pg inner join faction_id fi on fi.group_name=? "
						+ "where pg.group_id = fi.group_id and pg.type_id=?");
		removePermission = db
				.prepareStatement("delete from permissionByGroup where group_id IN (SELECT group_id FROM faction_id WHERE group_name = ?) and type_id=? and perm_id=?;");
		registerPermission = db
				.prepareStatement("insert into permissionIdMapping(perm_id,name) values(?,?);");
		getPermissionMapping = db
				.prepareStatement("select * from permissionIdMapping;");

		getPlayerTypeRoot = db
				.prepareStatement("select gpt.type_name from groupPlayerTypes gpt inner join faction_id on faction_id.group_id=gpt.group_id "
						+ "where gpt.type_id=0 and gpt.parent_id is null and faction_id.group_name=?;");

		addPlayerType = db
				.prepareStatement("insert into groupPlayerTypes (group_id,type_id,type_name, parent_id, blacklist) select group_id, ?,?,?,? from faction_id where group_name =?;");
		renamePlayerType = db
				.prepareStatement("update groupPlayerTypes gpt inner join faction_id fi on fi.group_id=gpt.group_id set gpt.type_name=? where gpt.type_id=? and fi.group_name=?;");
		removePlayerType = db
				.prepareStatement("delete gpt from groupPlayerTypes gpt inner join faction_id f on f.group_id=gpt.group_id where gpt.type_id=? and f.group_name=?;");
		removePlayerTypeMember = db
				.prepareStatement("delete fm from faction_member fm inner join faction_id fi on fi.group_id=fm.group_id where fi.group_name=? and fm.type_id=?;");
		removeInvitationByPlayerType = db
				.prepareStatement("delete from group_invitation where groupName=? and type_id=?;");
		getChildrenPlayerTypes = db
				.prepareStatement("select gpt.type_name, gpt.type_id, gpt.blacklist from groupPlayerTypes gpt inner join faction_id fi on fi.group_id=gpt.group_id where fi.group_name=? and gpt.parent_id=?;");
	...
	public int checkVersion(String name) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem accessing db_version table", e);
			// table doesnt exist
			return 0;
		}
	}
	...
	/**
	 * Updates the version number for a plugin. You must specify what the
	...
	public synchronized int updateVersion(int version, String pluginname) {
		...
		try {
			...
			plugin.getLogger()
					.log(Level.WARNING, "Problem updating version", e);
		}
		return ++version;
	}

	public synchronized int createGroup(String group, UUID owner,
			String password) {
		NameLayerPlugin.reconnectAndReintializeStatements();
		int ret = -1;
		try {
			...
			plugin.getLogger().log(Level.INFO,
					"Created group {0} w/ id {1} for {2}",
					new Object[] { group, ret, own });
		} catch (SQLException e) {
			plugin.getLogger().log(Level.WARNING,
					"Problem creating group " + group, e);
			ret = -1;
		}
	...
	public synchronized Group getGroup(String groupName) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting group " + groupName, e);
		}
		return null;
	}
	...
	public synchronized Group getGroup(int groupId) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting group " + groupId, e);
		}
		return null;
	}
	...
	public synchronized List<String> getGroupNames(UUID uuid) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting player's groups " + uuid, e);
		}
		return groups;
	}
	...
	public synchronized List<String> getGroupNames(UUID uuid, int typeId) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem getting player " + uuid + " groups by role "
							+ typeId, e);
		}
		return groups;
	}
	...
	public synchronized Timestamp getTimestamp(String group) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting group timestamp " + group, e);
		}
	...
	public synchronized int getPlayerTypeID(int groupid, UUID uuid) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem getting player " + uuid + " type within group "
							+ groupid, e);
		}
		return ptype;
	}
	...
	public synchronized void updateTimestamp(String group) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem updating timestamp for group " + group, e);
		}
	}
	...
	public synchronized void deleteGroup(String groupName) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem deleting group " + groupName, e);
		}
	}

	public synchronized void addMember(UUID member, String faction,
			PlayerType role) {
		NameLayerPlugin.reconnectAndReintializeStatements();
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem adding " + member + " as " + role + " to group "
							+ faction, e);
		}
	}

	public synchronized Map<UUID, PlayerType> getAllMembers(String groupName,
			PlayerTypeHandler handler) {
		NameLayerPlugin.reconnectAndReintializeStatements();
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting all members" + " for group " + groupName,
					e);
		}
		return members;
	}
	...
	public synchronized void removeMember(UUID member, String group) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem removing " + member + " from group " + group, e);
		}
	}
	...
	public synchronized void addSubGroup(String group, String subGroup) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem adding subgroup " + subGroup + " to group "
							+ group, e);
		}
	}
	...
	public synchronized List<Group> getSubGroups(String group) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting subgroups for group " + group, e);
		}
		return groups;
	}
	...
	public synchronized Group getSuperGroup(String group) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting superGroup for group " + group, e);
		}
		return null;
	}
	...
	public synchronized void removeSubGroup(String group, String subGroup) {
		...
		try {
			...
			plugin.getLogger()
					.log(Level.WARNING,
							"Removing subgroup " + subGroup + " from group "
									+ group, e);
		}
	}

	public synchronized void addPermission(String groupName, PlayerType role,
			List<PermissionType> perms) {
		NameLayerPlugin.reconnectAndReintializeStatements();
		for (PermissionType perm : perms) {
			try {
				...
				plugin.getLogger().log(
						Level.WARNING,
						"Problem adding " + role + " with " + perms
	...
	public synchronized PlayerTypeHandler getPermissions(Group group) {
		...
		try {
			...
			PlayerType root = new PlayerType(rootName, 0, null, group, false);
			handler = new PlayerTypeHandler(root, group, false);
			recursivelyLoadPlayerType(group, handler, root);
		} catch (SQLException e) {
			plugin.getLogger().log(Level.WARNING,
					"Problem getting permissions for group " + group, e);
		}
		return handler;
	}
	...
	public synchronized void addPlayerType(Group g, PlayerType type) {
		...
		try {
			...
			if (type.getParent() == null) {
				addPlayerType.setNull(3, java.sql.Types.INTEGER);
			}
			else {
				addPlayerType.setInt(3, type.getParent().getId());
			}
			addPlayerType.setBoolean(4, type.isBlacklistType());
			addPlayerType.setString(5, g.getName());
			addPlayerType.execute();
		} catch (SQLException e) {
			plugin.getLogger().log(
					Level.WARNING,
					"Problem adding player type " + type.getName()
							+ " for group" + g.getName(), e);
		}
	}
	...
	public synchronized void removePlayerType(Group g, PlayerType type) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem removing player type " + type.getName()
							+ " for group" + g.getName(), e);
		}
	}
	...
	public synchronized void updatePlayerTypeName(Group g, PlayerType type) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem updating player type name for " + type.getName()
							+ " with id " + type.getId() + " for group"
							+ g.getName(), e);
		}
	}

	private void recursivelyLoadPlayerType(Group group,
			PlayerTypeHandler handler, PlayerType parentVertex) {
		try {
			...
			Map<String, Boolean> isBlackListMap = new HashMap<String, Boolean>();
			while (childSet.next()) {
				String childName = childSet.getString(1);
				int childId = childSet.getInt(2);
				boolean black = childSet.getBoolean(3);
				isBlackListMap.put(childName, black);
				children.put(childName, childId);
			}
			childSet.close();
			for (Entry<String, Integer> entry : children.entrySet()) {
				getPermission.setString(1, group.getName());
				getPermission.setInt(2, entry.getValue());
				...
				PlayerType child = new PlayerType(entry.getKey(), entry.getValue(),
						parentVertex, permList, group, isBlackListMap.get(entry.getKey()));
			...
			plugin.getLogger()
					.log(Level.WARNING,
							"Problem getting player types for group "
									+ group.getName(), e);
		}
	}

	public synchronized void removePermission(String group, PlayerType pType,
			PermissionType perm) {
		NameLayerPlugin.reconnectAndReintializeStatements();
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem removing permissions for group " + group
	...
	public synchronized void registerPermission(PermissionType perm) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem register permission " + perm.getName(), e);
		}
	}
	...
	public synchronized Map<Integer, String> getPermissionMapping() {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting permissions from db", e);
		}
		return perms;
	}
	...
	public synchronized int countGroups(UUID uuid) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem counting groups for " + uuid, e);
		}
		return 0;
	...
	public synchronized void mergeGroup(String groupName, String toMerge) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem merging group " + toMerge + " into " + groupName,
					e);
		}
	}
	...
	public synchronized void updatePassword(String groupName, String password) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem updating password for group " + groupName, e);
		}
	}
	...
	public synchronized void autoAcceptGroups(UUID uuid) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem setting autoaccept for " + uuid, e);
		}
	}
	...
	public synchronized boolean shouldAutoAcceptGroups(UUID uuid) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting autoaccept for " + uuid, e);
		}
		return false;
	}
	...
	public synchronized void removeAutoAcceptGroup(UUID uuid) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem removing autoaccept for " + uuid, e);
		}
	}
	...
	public synchronized void setDefaultGroup(UUID uuid, String groupName) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem setting user " + uuid + " default group to "
							+ groupName, e);
		}
	}
	...
	public synchronized void changeDefaultGroup(UUID uuid, String groupName) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem changing user " + uuid + " default group to "
							+ groupName, e);
		}
	}
	...
	public synchronized String getDefaultGroup(UUID uuid) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting default group for " + uuid, e);
		}
		return null;
	}
	...
	public synchronized Map<UUID, String> getAllDefaultGroups() {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem getting all default groups ", e);
		}
		return groups;
	}
	 ...
	 * @param group
	 *            This is the group that we are changing the founder of.
	 */
	public synchronized void setFounder(UUID uuid, Group group) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem setting founder of group " + group.getName()
							+ " to " + uuid, e);
		}
	}

	public synchronized void addGroupInvitation(UUID uuid, String groupName,
			PlayerType role) {
		NameLayerPlugin.reconnectAndReintializeStatements();
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem adding group " + groupName + " invite for " + uuid
							+ " with role " + role.getName(), e);
		}
	}
	...
	public synchronized void removeGroupInvitation(UUID uuid, String groupName) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem removing group " + groupName + " invite for "
							+ uuid, e);
		}
	}

	/**
	 * Use this method to load a specific invitation to a group without the
	 * notification.
	 * 
	 * @param playerUUID
	 *            The uuid of the invited player.
	 * @param group
	 *            The group the player was invited to.
	 */
	public synchronized void loadGroupInvitation(UUID playerUUID, Group group) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem loading group " + group.getName() + " invite for "
							+ playerUUID, e);
		}
	}
	...
	public synchronized Map<UUID, PlayerType> getInvitesForGroup(Group g) {
		...
		try {
			...
			plugin.getLogger().log(
					Level.WARNING,
					"Problem loading group invitations for group "
							+ g.getName(), e);
		}
		return invs;
	}
	...
	public synchronized void loadGroupsInvitations() {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Problem loading all group invitations", e);
		}
	}

	public synchronized void logNameChange(UUID uuid, String oldName,
			String newName) {
		NameLayerPlugin.reconnectAndReintializeStatements();
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Failed to log a name change for {0} from {1} -> {2}",
					new Object[] { uuid, oldName, newName });
			plugin.getLogger()
					.log(Level.WARNING, "Exception during change.", e);
		}
	}
	...
	public synchronized boolean hasChangedNameBefore(UUID uuid) {
		...
		try {
			...
			plugin.getLogger()
					.log(Level.WARNING,
							"Failed to check if {0} has previously changed names",
							uuid);
	 ...
	 * Ideally only one groupname/id has members and the rest are shadows, but
	 * in any case we arbitrarily define primacy as the one with the most
	 * members for ease of accounting and backwards compatibility.
	 * 
	 * @param groupName
	 * @return
	 */
	public List<Integer> getAllIDs(String groupName) {
		...
		try {
			...
			plugin.getLogger().log(Level.WARNING,
					"Unable to fully load group ID set", se);
		}
		return null;
	}
